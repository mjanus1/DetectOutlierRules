package com.mariusz.janus.DetectOutlierRules.domain;

public class ServerProperty {
	public static final String CLIENT_ID = "Test";
	public static final String CLIENT_SECRET = "zaqwsx";
	public static final String SERVER_URL = "http://les.szymon.net.pl";
	public static final String AUTHORIZE_PATH = "/oauth/authorize";
	public static final String REDIRECT_URL = "http://localhost:8000/DetectOutlierRules/auth.xhtml";
	public static final String TOKEN_PATH = "/oauth/token";
	public static final String ACCOUNT = "/api/account";
	public static final String KNOWLEDGEBASE = "/api/knowledgeBases/";
	public static final String RULES_COUNT = "/rules/count";
	public static final String ATTRIBUTES_COUNT = "/attributes/count";
	public static final String FACTS_COUNT = "/facts/count";
	public static final String AUTHORIZATION = "Authorization";
	public static final String BEARER = "Bearer";
	public static final String FACTS = "/facts";
	public static final String ATTRIBUTES = "/attributes";
	public static final String ALL_RULES = "/rules?all=true";
	public static final String SEPARATOR = "/";
}